		...
		descriptions: {
			...
			stream: {
				stretch: {
					...
					extendingOnly: "ノートがクリップよりも長い場合はストレッチします。それ以外の場合は、長さを短くします",
					flexingOnly: "ノートがクリップより短い場合はストレッチします。それ以外の場合は、長さを長くします",
				},
				loop: {
				...
				truncateAndLegatoConflictInAudio: "トランクションとレガートがオーディオで競合しているため、同時に有効にすることはできません！",
			...
			shupelunker: {
				...
				unallocated: {
					...
					octaves: "最も近い高オクターブまたは低いオクターブのクリップを使用します。これは最も優先度が高く、ソングソルフェ<unk> ges を持つソースに便利です。",
